    ...
    for (int i = 0; i < num_groups; i++) {
        ...
        if (type == "rotation") {
            ...
            throw std::runtime_error(
                "MeshMotion: Invalid mesh motion type: " + type);
        }
    }

    if (node["start_time"]) startTime_ = node["start_time"].as<double>();
...
void MeshMotion::initialize()
{
    init_coordinates();
    if (startTime_ > 0.0)
        for (auto& mm : meshMotionVec_) mm->initialize(startTime_);
}
...
void MeshMotion::execute(const int istep)
{
    ...
    for (auto& mm : meshMotionVec_) mm->execute(curr_time);
}
...
void MeshMotion::init_coordinates()
{
    ...
    for (auto b : bkts) {
        for (size_t in = 0; in < b->size(); in++) {
            ...
            for (int d = 0; d < ndim; d++) xyz[d] = oldxyz[d];
        }
    }
}
