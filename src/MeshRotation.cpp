...
void MeshRotation::execute(double current_time) { rotate_mesh(current_time); }
...
void MeshRotation::rotate_mesh(double current_time)
{
    ...
    double mag = 0.0;
    for (int d = 0; d < ndim; d++) mag += axis_[d] * axis_[d];
    mag = std::sqrt(mag);
    ...
    for (auto b : bkts) {
        for (size_t in = 0; in < b->size(); in++) {
            ...
            for (int d = 0; d < ndim; d++) dx[d] = xyz[d] - oldxyz[d];
        }
    }
}
