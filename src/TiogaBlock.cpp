...
void TiogaBlock::setup()
{
    ...
    if (wallNames_.size() > 0) names_to_parts(wallNames_, wallParts_);
...
void TiogaBlock::update_connectivity()
{
    process_nodes();
    process_wallbc();
    process_ovsetbc();
...
void TiogaBlock::update_iblanks()
{
    ...
    if (bulk_.parallel_rank() == 0) {
        std::cout << "STK IBLANK " << meshtag_ << ": field=" << gcounter[0]
                  << "; fringe=" << gcounter[1] << "; hole=" << gcounter[2]
                  << std::endl;
    }
}
...
void TiogaBlock::update_iblank_cell()
{
    ...
    stk::mesh::Selector mesh_selector =
        meta_.locally_owned_part() & stk::mesh::selectUnion(blkParts_);
...
void TiogaBlock::get_donor_info(
    TIOGA::tioga& tg, stk::mesh::EntityProcVec& egvec)
{
    ...
    for (int i = 0; i < (4 * dcount); i += 4) {
        ...
        auto elemID =
            bdata_.cell_gid_.h_view[elemid_tmp]; // Global ID of element
        ...
        stk::mesh::Entity elem =
            bulk_.get_entity(stk::topology::ELEM_RANK, elemID);
...
{
    stk::mesh::Selector mesh_selector =
        stk::mesh::selectUnion(wallParts_) &
        (meta_.locally_owned_part() | meta_.globally_shared_part());
    const stk::mesh::BucketVector& mbkts =
        bulk_.get_buckets(stk::topology::NODE_RANK, mesh_selector);
...
{
    stk::mesh::Selector mesh_selector =
        stk::mesh::selectUnion(ovsetParts_) &
        (meta_.locally_owned_part() | meta_.globally_shared_part());
    const stk::mesh::BucketVector& mbkts =
        bulk_.get_buckets(stk::topology::NODE_RANK, mesh_selector);
...
void TiogaBlock::process_elements()
{
    ...
    if (tioga_conn_) delete[] tioga_conn_;
...
void TiogaBlock::compute_volumes()
{
    ...
    for (size_t n = 0; n < ntypes; ++n) {
        ...
        for (int i = 0; i < nc[n]; ++i) {
            ...
            const double vol = computeCellVolume(xv, nvert);
            {
                ...
                const auto elem =
                    bulk_.get_entity(stk::topology::ELEM_RANK, eid);
            ...
            for (int m = 0; m < nvert; ++m) {
                ...
                const auto node =
                    bulk_.get_entity(stk::topology::NODE_RANK, nid);
    ...
    stk::mesh::Selector sel =
        stk::mesh::selectUnion(blkParts_) &
        (meta_.locally_owned_part() | meta_.globally_shared_part());
    const auto& mbkts = bulk_.get_buckets(stk::topology::NODE_RANK, sel);
...
void TiogaBlock::adjust_cell_resolutions()
{
    ...
    if (bulk_.parallel_rank() == 0) {
        std::cout << "Set resolutions for mandatory fringes:\n    "
                  << "Mesh ID = " << meshtag_ << " Elements = " << gcounter[0]
                  << " Nodes = " << gcounter[1] << std::endl;
    }
...
void TiogaBlock::register_block_classic(TIOGA::tioga& tg)
{
    ...
    tg.registerGridData(
        ...
        bdata_.num_verts_.h_view
            .size(), // Number of topologies in this mesh block
    ...
    tg.setResolutions(
        meshtag_, bdata_.node_res_.h_view.data(),
        bdata_.cell_res_.h_view.data());
}
...
void TiogaBlock::register_solution(
    TIOGA::tioga& tg, const int nvars, const bool use_ngp_iface)
{
    if (num_nodes_ < 1) return;
    ...
    for (auto b : bkts) {
        for (size_t in = 0; in < b->size(); ++in) {
            ...
            for (int i = 0; i < nvars; ++i) qsolarr(ip++) = qq[i];
        }
    }
...
{
    // Note solution registration is handled separately. We don't process that
    // here.
    static_assert(
        NgpTiogaBlock::max_vertex_types ==
            TIOGA::MeshBlockInfo::max_vertex_types,
