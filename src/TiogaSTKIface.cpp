...
void TiogaSTKIface::load(const YAML::Node& node)
{
    ...
    if (node["use_adaptive_hole_map"]) {
        ...
        if (use_adaptive_hole_map_ == true) tg_.setHoleMapAlgorithm(1);
    }
}
...
void TiogaSTKIface::execute()
{
    ...
    for (auto& tb : blocks_) {
        ...
        // For each block determine donor elements that needs to be ghosted to
        // other MPI ranks
...
void TiogaSTKIface::update_ghosting()
{
    ...
    if ((global[0] > 0) || (global[1] > 0)) {
        ...
        bulk_.change_ghosting(
            *ovsetGhosting_, elemsToGhost_, recvGhostsToRemove);
        bulk_.modification_end();

        sierra::nalu::populate_ghost_comm_procs(
            bulk_, *ovsetGhosting_, ghostCommProcs_);
    }
#endif
}
...
void TiogaSTKIface::check_soln_norm()
{
    ...
    if (bulk_.parallel_rank() == 0)
        std::cout << "TIOGA Interpolation error: max L2 error: " << g_maxNorm
                  << std::endl;
}

void TiogaSTKIface::get_receptor_info()
{
    ScalarFieldType* ibf =
        meta_.get_field<ScalarFieldType>(stk::topology::NODE_RANK, "iblank");
    ...
    stk::mesh::EntityId donorID =
        std::numeric_limits<stk::mesh::EntityId>::max();
    ...
    for (size_t i = 0; i < ncount; i += rec_offset) {
        ...
        int nodeID =
            blocks_[mtag]
                ->node_id_map()[nid]; // STK Global ID of the fringe node
        stk::mesh::Entity node =
            bulk_.get_entity(stk::topology::NODE_RANK, nodeID);

        if (!bulk_.bucket(node).owned()) {
            // We have a shared node that is marked as fringe. Ensure that the
            // owning proc also has this marked as fringe.
            double ibval = *stk::mesh::field_data(*ibf, node);

            if (ibval > -1.0) {
                // Disagreement between owner and shared status of iblank.
                // Communicate to owner that it must be a fringe.
    ...
    for (auto ielm : nbPerProc) nTotalEntities += ielm;
...
#if 1
    if (iproc == 0)
        std::cout << "TIOGA: Detected fringe/field mismatch on "
                  << (nTotalEntities / 3) << " entities" << std::endl;
#endif
    ...
    for (int i = 0; i < nTotalEntities; i += 3) {
        ...
        if (bulk_.is_valid(elem) &&
            (bulk_.parallel_owner_rank(elem) == iproc) && (nodeProc != iproc)) {
...
void TiogaSTKIface::populate_overset_info()
{
#ifdef HAS_NALU_WIND
    ...
    if (nearestDistance > (1.0 + 1.0e-8)) {
        ...
        meSCS->interpolatePoint(
            3, info.isoCoords_.data(), elemxyz.data(), intxyz.data());
    ...
    if (iproc == 0)
        std::cout << "Nalu CVFEM interpolation results: max error = "
                  << g_maxError << std::endl;
#endif
...
void TiogaSTKIface::register_solution(const int nvars)
{
    ...
    for (auto& tb : blocks_) tb->register_solution(tg_, nvars, use_ngp_iface_);
}
...
void TiogaSTKIface::update_solution(const int nvars)
{
    ...
    if (bulk_.parallel_rank() == 0) {
        ...
        if (g_maxNorm >= err_tol) {
            throw std::runtime_error(
                "TIOGA interpolation error for STK mesh is above the set "
