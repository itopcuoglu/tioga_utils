...
private:
     ...
     *  The method is also responsible for determining fringe/field mismatches
     * for the shared nodes across processor interfaces. The logic used is to
     * signal all procs sharing the node to take on a fringe status (iblank =
     * -1) if one of them has status of fringe while others have a status of
     * field point.
     */
