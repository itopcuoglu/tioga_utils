...
FieldBase<amrex::MultiFab>::operator()(int lev) const noexcept
{
    BL_ASSERT(lev < m_repo.num_active_levels());
    return m_repo.get_multifab(m_id, lev);
}
...
FieldBase<amrex::iMultiFab>::operator()(int lev) const noexcept
{
    BL_ASSERT(lev < m_repo.num_active_levels());
    return m_repo.get_int_fab(m_id, lev);
}
...
FieldBase<amrex::MultiFab>::vec_const_ptrs() const noexcept
{
    const int nlevels = m_repo.num_active_levels();
...
FieldBase<amrex::iMultiFab>::vec_ptrs() noexcept
{
    const int nlevels = m_repo.num_active_levels();
...
FieldBase<amrex::iMultiFab>::vec_const_ptrs() const noexcept
{
    const int nlevels = m_repo.num_active_levels();
    ...
    const amrex::FabFactory<amrex::FArrayBox>& factory)
{
    ...
    for (auto& field : m_field_vec) {
        auto ba1 = amrex::convert(ba, index_type(field->field_location()));
