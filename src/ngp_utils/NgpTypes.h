...
struct NgpDualArray
{
    ...
    void sync_to_device() { Kokkos::deep_copy(d_view, h_view); }

    void sync_to_host() { Kokkos::deep_copy(h_view, d_view); }
};

template <typename Mesh = stk::mesh::NgpMesh>
struct NGPMeshTraits
{
    using TeamPolicy = Kokkos::
        TeamPolicy<typename Mesh::MeshExecSpace, stk::ngp::ScheduleType>;
