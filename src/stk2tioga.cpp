...
void tag_procs(stk::mesh::MetaData& meta, stk::mesh::BulkData& bulk)
{
    ...
    ScalarFieldType* ipnode =
        meta.get_field<ScalarFieldType>(stk::topology::NODE_RANK, "pid_node");
    ScalarFieldType* ipelem =
        meta.get_field<ScalarFieldType>(stk::topology::ELEM_RANK, "pid_elem");

    stk::mesh::Selector msel = meta.locally_owned_part();
    const stk::mesh::BucketVector& nbkts =
        bulk.get_buckets(stk::topology::NODE_RANK, msel);
    ...
    const stk::mesh::BucketVector& ebkts =
        bulk.get_buckets(stk::topology::ELEM_RANK, msel);
...
void move_mesh(stk::mesh::MetaData& meta, stk::mesh::BulkData& bulk)
{
    ...
    stk::mesh::BucketVector bkts =
        bulk.get_buckets(stk::topology::NODE_RANK, mselect);
    for (auto b : bkts) {
        for (size_t in = 0; in < b->size(); in++) {
    ...
    const double time = 0.0)
{
    ...
    if (inpfile["motion_info"]) has_motion = true;

    tag_procs(meta, bulk);
    ScalarFieldType* ibf =
        meta.get_field<ScalarFieldType>(stk::topology::NODE_RANK, "iblank");
...
int main(int argc, char** argv)
{
...

    {
        ...
        if ((nproc > 1) && inpfile["decomposition_method"]) {
            auto decomp_method =
                inpfile["decomposition_method"].as<std::string>();
            stkio.property_add(
        ...
        if (iproc == 0) std::cout << "Calling TIOGA setup... " << std::endl;
        ...
        if (iproc == 0) std::cout << "Loading mesh... " << std::endl;
        {
            auto timeMon2 =
                tioga_nalu::get_timer("stk2tioga::populate_bulk_data");
            stkio.populate_bulk_data();
        }

        if (iproc == 0) std::cout << "Initializing TIOGA... " << std::endl;
        if (has_motion) mesh_motion->initialize();
        tg.initialize();

        if (iproc == 0)
            std::cout << "Performing initial overset connectivity... "
                      << std::endl;
        tg.execute();
        tg.check_soln_norm();
        ...
        if (has_motion) {
            ...
            if (iproc == 0)
                std::cout << "Execution mesh motion for num steps = " << nsteps
                          << std::endl
                          << std::endl;
        ...
        if (dump_partitions) {
            auto timeMon4 =
                tioga_nalu::get_timer("stk2tioga::dump_tioga_partitions");
            if (iproc == 0)
